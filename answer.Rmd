...
The combination between supervised and unsupervised learning is semi-supervised learning. It is a class of supervised learning that can be used when the target variable has very limited labeled data by utilizing unsupervised learning to generate clusters or pseudo-label of the data. 
...
Using only smartphone and smartwatch, it is possible to label the activity of the user manually. There are some android apps that can be used to log activity. One of the open-source apps can be used via this [link](https://github.com/nlathia/pydata_2016/tree/master/App/AccelerometerCollector). 

However, sometimes the target label is not exist. In this case, the label of activity is not exist. Hence, using unsupervised or semi-supervised learning approach can be the alternative. Unsupervised learning can be used to identifying hidden pattern from the data. In addition, when we have very limited target label, we can use semi-supervised approach.

In the end, even though we use unspervised or semi-supervised learning, the purpose is stay same
